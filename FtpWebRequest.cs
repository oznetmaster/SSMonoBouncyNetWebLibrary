...
namespace System.Net
	...
	public sealed class FtpWebRequest : WebRequest
...
#if SSHARP
					if (!Int64Ex.TryParse (desc.Substring (4, len), out size))
#else
		...
		private Socket SetupPassiveConnection (string statusDescription)
			...
			int p1, p2, port;
#if SSHARP
			if (!Int32Ex.TryParse (digits[4], out p1) || !Int32Ex.TryParse (digits[5], out p2))
#else
		...
		internal FtpStatus GetResponseStatus ()
			...
			while (true)
				...
				int code;
#if SSHARP
				if (!Int32Ex.TryParse (response.Substring (0, 3), out code))
#else
